...
const convertToBedrock = async version => {
  ...
  const [kzPng] = await Promise.all([
    ...
    zip.addLocalFolderPromise(
      `${CENSORED_DIR}/${BASE_PACK_DIR}/${POOF_NAMESPACE}/sounds/`,
...
const generateManifest = (version, isBeta) => {
  ...
  return {
    ...
    subpacks: [
      { folder_name: CENSORED_DIR, name: 'Censored', memory_tier: 0 },
      { folder_name: NORMAL_SUBPACK, name: 'Normal', memory_tier: 0 },
    ],
    settings: [
      { type: 'label', text: "Move slider to 'Censored' to reduce profanity" },
    ],
  };
};
